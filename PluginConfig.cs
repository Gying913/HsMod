...
namespace HsMod
{
    public static class PluginConfig
    {
        ...
        // 英雄皮肤映射（支持多值随机）
        public static Dictionary<int, List<int>> HeroesMapping = new Dictionary<int, List<int>>();
        ...
        public static void ConfigBind(ConfigFile config)
        {
            ...
            isTimeGearEnable = config.Bind(LocalizationManager.GetLangValue("isTimeGearEnable.label"), LocalizationManager.GetLangValue("isTimeGearEnable.name"), true, LocalizationManager.GetLangValue("isTimeGearEnable.description"));
           
            timeGear = config.Bind(LocalizationManager.GetLangValue("timeGear.label"),LocalizationManager.GetLangValue("timeGear.name"),4f,  new ConfigDescription(LocalizationManager.GetLangValue("timeGear.description"),new AcceptableValueList<float>(2f, 4f, 8f, 16f, 32f)));// 齿轮下拉选项列表
            ...
            targetFrameRate = config.Bind(LocalizationManager.GetLangValue("targetFrameRate.label"), LocalizationManager.GetLangValue("targetFrameRate.name"), 144, new ConfigDescription(LocalizationManager.GetLangValue("targetFrameRate.description"), new AcceptableValueRange<int>(-1, 2333)));
            ...
            isShowCollectionCardIdEnable = config.Bind(LocalizationManager.GetLangValue("isShowCollectionCardIdEnable.label"), LocalizationManager.GetLangValue("isShowCollectionCardIdEnable.name"), true, LocalizationManager.GetLangValue("isShowCollectionCardIdEnable.description"));
            ...
            isFullnameShow = config.Bind(LocalizationManager.GetLangValue("isFullnameShow.label"), LocalizationManager.GetLangValue("isFullnameShow.name"), true, LocalizationManager.GetLangValue("isFullnameShow.description"));
            isOpponentRankInGameShow = config.Bind(LocalizationManager.GetLangValue("isOpponentRankInGameShow.label"), LocalizationManager.GetLangValue("isOpponentRankInGameShow.name"), true, LocalizationManager.GetLangValue("isOpponentRankInGameShow.description"));
            isCardTrackerEnable = config.Bind(LocalizationManager.GetLangValue("isCardTrackerEnable.label"), LocalizationManager.GetLangValue("isCardTrackerEnable.name"), true, LocalizationManager.GetLangValue("isCardTrackerEnable.description"));
            isCardRevealedEnable = config.Bind(LocalizationManager.GetLangValue("isCardRevealedEnable.label"), LocalizationManager.GetLangValue("isCardRevealedEnable.name"), true, LocalizationManager.GetLangValue("isCardRevealedEnable.description"));
            ...
            goldenCardState = config.Bind(LocalizationManager.GetLangValue("goldenCardState.label"), LocalizationManager.GetLangValue("goldenCardState.name"), Utils.CardState.OnlyMy, LocalizationManager.GetLangValue("goldenCardState.description"));
            maxCardState = config.Bind(LocalizationManager.GetLangValue("maxCardState.label"), LocalizationManager.GetLangValue("maxCardState.name"), Utils.CardState.OnlyMy, LocalizationManager.GetLangValue("maxCardState.description"));
            ...
            mercenaryDiamondCardState = config.Bind(LocalizationManager.GetLangValue("mercenaryDiamondCardState.label"), LocalizationManager.GetLangValue("mercenaryDiamondCardState.name"), Utils.CardState.OnlyMy, LocalizationManager.GetLangValue("mercenaryDiamondCardState.description"));
            ...
            isBgsSeasonTicketUnlock = config.Bind(LocalizationManager.GetLangValue("isBgsSeasonTicketUnlock.label"), LocalizationManager.GetLangValue("isBgsSeasonTicketUnlock.name"), true, LocalizationManager.GetLangValue("isBgsSeasonTicketUnlock.description"));
            ...
            shieldMainBoxLuckyDraw = config.Bind(LocalizationManager.GetLangValue("shieldMainBoxLuckyDraw.label"), LocalizationManager.GetLangValue("shieldMainBoxLuckyDraw.name"), true, LocalizationManager.GetLangValue("shieldMainBoxLuckyDraw.description"));
            ...
            skinCardBack = config.Bind(LocalizationManager.GetLangValue("skinCardBack.label"), LocalizationManager.GetLangValue("skinCardBack.name"), 20, LocalizationManager.GetLangValue("skinCardBack.description"));
            ...
            webServerPort = config.Bind(LocalizationManager.GetLangValue("webServerPort.label"), LocalizationManager.GetLangValue("webServerPort.name"), 3155, new ConfigDescription(LocalizationManager.GetLangValue("webServerPort.description"), new AcceptableValueRange<int>(1, 65535)));
            webPageBackImg = config.Bind(LocalizationManager.GetLangValue("webPageBackImg.label"), LocalizationManager.GetLangValue("webPageBackImg.name"), "https://wp.upx8.com/api.php", new ConfigDescription(LocalizationManager.GetLangValue("webPageBackImg.description"), null, new object[] { "Advanced" }));
            ...
            fakePackCount = config.Bind(LocalizationManager.GetLangValue("fakePackCount.label"), LocalizationManager.GetLangValue("fakePackCount.name"), 8888, LocalizationManager.GetLangValue("fakePackCount.description"));
        ...
        public static void LoadSkinsConfigFromFile()
        {
            ...
            // 硬编码基础映射
            HeroesMapping.Add(274, new List<int> { 119049 }); // 德鲁伊
            HeroesMapping.Add(637, new List<int> { 108389 }); // 法师
            HeroesMapping.Add(78065, new List<int> { 93448 }); // DK
            HeroesMapping.Add(56550, new List<int> { 114192 }); // 恶魔猎手
            HeroesMapping.Add(31, new List<int> { 123733 }); // 猎人
            HeroesMapping.Add(671, new List<int> { 123730 }); // 圣骑士
            HeroesMapping.Add(813, new List<int> { 103777 }); // 牧师
            HeroesMapping.Add(930, new List<int> { 121643 }); // 贼
            HeroesMapping.Add(893, new List<int> { 103104 }); // 术士
            HeroesMapping.Add(7, new List<int> { 119076 }); // 战士
            HeroesMapping.Add(1066, new List<int> { 111235 }); // 萨满

            // 读取配置文件
            string configFile = Path.Combine(BepInEx.Paths.ConfigPath, "HsSkins.cfg");
            if (File.Exists(configFile))
            {
                try
                {
                    ...
                    foreach (string line in lines)
                    {
                        ...
                        string[] parts = trimLine.Split(':', StringSplitOptions.RemoveEmptyEntries);
                        if (parts.Length < 2)
                            continue;

                        if (!int.TryParse(parts[0].Trim(), out int heroId))
                            continue;

                        List<int> skinIds = parts[1]
                        ...
                        if (HeroesMapping.ContainsKey(heroId))
                            HeroesMapping[heroId] = skinIds;
                        else
                            HeroesMapping.Add(heroId, skinIds);
                    }
                }
                catch (Exception ex)
                {
                    Utils.MyLogger(BepInEx.Logging.LogLevel.Error, $"读取皮肤配置文件失败：{ex.Message}");
                }
            }
            else
            {
            ...
            {
                "# 皮肤映射表",
                "# 说明：主要用作英雄（酒馆、对战）类皮肤替换；修改ID后按下F4刷新并在BepInEx目录下生成当前全部皮肤信息",
                "# 格式1：原始皮肤ID:替换皮肤ID（:为半角字符）",
                "# 格式2：原始皮肤ID:皮肤ID1,皮肤ID2（随机切换多个皮肤）",
                "# 格式3：274德鲁伊、637法师、78065死亡骑士、56550恶魔猎手、31猎人、671圣骑士、813牧师、930盗贼、893术士、7战士、1066萨满",
                "# 示例：①274:57761,119049 （德鲁伊随机使用两个皮肤）; ②注释行以#开头，③空行不影响解析",
                ""

            };

                    foreach (var kvp in HeroesMapping)
                    {
                        string skinStr = string.Join(",", kvp.Value);
                        defaultLines.Add($"{kvp.Key}:{skinStr}");
                    }
                    File.WriteAllLines(configFile, defaultLines);
                    Utils.MyLogger(BepInEx.Logging.LogLevel.Info, $"生成默认皮肤配置文件：{configFile}");
                }
                catch (Exception ex)
                {
                    Utils.MyLogger(BepInEx.Logging.LogLevel.Error, $"生成皮肤配置文件失败：{ex.Message}");
                }
            }
        }
