    ...
    public class Patcher
    {
        ...
        public class PatchHearthstone
        {
            ...
            public static void PatchDebugPrintPower(GameState __instance, ref List<Network.PowerHistory> powerList)
            {
                try
                {
                    ...
                    // 定义炉石核心TAG常量（修复CS0103错误）
                    const int TAG_CHOICE_ID = 108;
                    const int TAG_CHOICE_COUNT = 109;  // 修复：补充定义
                    const int TAG_CHOICE_ACTIVE = 49;
                    const int TAG_DISCOVER = 39;       // 修复：补充定义
                    const int TAG_CHOICE_TYPE = 447;   // 修复：补充定义
                    ...
                    foreach (var powerHistory in powerList)
                    {
                        ...
                        // 修复CS0426：使用正确的历史实体类型（带Hist前缀）
                        Network.Entity netEntity = null;
                        if (powerHistory.Type == Network.PowerType.SHOW_ENTITY)
                        {
                            var histShowEntity = powerHistory as Network.HistShowEntity;
                            netEntity = histShowEntity?.Entity;
                        }
                        else if (powerHistory.Type == Network.PowerType.FULL_ENTITY)
                        {
                            var histFullEntity = powerHistory as Network.HistFullEntity;
                            netEntity = histFullEntity?.Entity;
                        }
                        else if (powerHistory.Type == Network.PowerType.TAG_CHANGE)
                        {
                            // 修复CS0136：变量名冲突，将内部entity重命名为innerEntity
                            var histTagChange = powerHistory as Network.HistTagChange;
                            if (histTagChange != null)
                            {
                                Entity innerEntity = __instance?.GetEntity(histTagChange.Entity);
                                //netEntity = innerEntity?.GetNetworkEntity(); // 若仍报错，直接注释此行
                            }
                        }

                        if (netEntity == null || string.IsNullOrEmpty(netEntity.CardID))
                            continue;

                        // 获取外部实体（与内部innerEntity区分，修复变量冲突）
                        ...
                        if (entityDef == null || entityDef.IsQuestline())
                            continue;

                        // 修复Tag类型引用和判断逻辑
                        bool isChoiceCard = false;
                        if (netEntity.Tags != null && netEntity.Tags.Count > 0)
                        {
                            // 遍历实体标签（Tag类型为Network.Entity.Tag）
                            isChoiceCard = netEntity.Tags.Any(tg =>
                                (tg.Name == TAG_CHOICE_ACTIVE && tg.Value == 4) ||
                                tg.Name == TAG_CHOICE_ID ||
                                tg.Name == TAG_CHOICE_COUNT ||
                                (tg.Name == TAG_DISCOVER && tg.Value == 1) ||
                                (tg.Name == TAG_CHOICE_TYPE && tg.Value > 0));
                        }

                        bool isChoiceZone = entity.GetZone() == TAG_ZONE.SETASIDE
                                          || entity.GetZone() == TAG_ZONE.HAND
                                          || entity.GetZone() == TAG_ZONE.PLAY
                                          || entity.GetZone() == TAG_ZONE.DECK
                                          || entity.GetZone() == TAG_ZONE.SECRET;

                        if (isChoiceCard && isChoiceZone)
                        {
                            string cardId = netEntity.CardID;
                            string cardName = entityDef.GetName() ?? $"未知卡牌（ID:{cardId}）";
                            string cardDesc = entityDef.GetCardTextInHand() ?? "无描述";

                            if (!uniqueChoices.Contains(cardId))
                            {
                                string choiceType = GetChoiceType(netEntity.Tags, TAG_DISCOVER, TAG_CHOICE_COUNT, TAG_CHOICE_TYPE);
                                string hint = $"{choiceType}: {cardName}\n{cardDesc}";

                                uniqueChoices.Add(cardId);
                                UIStatus.Get().AddInfo(hint, 15f);
                                Utils.MyLogger(BepInEx.Logging.LogLevel.Info, $"追踪到{choiceType}: {cardName}（ID:{cardId}）");
                            }
                        }
                    }
                }
                ...
                {
                    Utils.MyLogger(BepInEx.Logging.LogLevel.Error, $"抉择识别异常: {ex.Message}\n{ex.StackTrace}");
                }
            }

            // 辅助方法：通过参数传递TAG常量（避免上下文错误）
            private static string GetChoiceType(List<Network.Entity.Tag> tags, int tagDiscover, int tagChoiceCount, int tagChoiceType)
            {
                if (tags == null) return "抉择";

                var discoverTag = tags.FirstOrDefault(t => t.Name == tagDiscover);
                if (discoverTag != null && discoverTag.Value == 1)
                    return "发现选项";

                var countTag = tags.FirstOrDefault(t => t.Name == tagChoiceCount);
                if (countTag != null && countTag.Value == 3)
                    return "3选1选项";

                var typeTag = tags.FirstOrDefault(t => t.Name == tagChoiceType);
                if (typeTag != null && typeTag.Value > 0)
                    return "特殊抉择";
        ...
        public class PatchLogArchive
        {
            ...
            private static void PatchUpdateCornerReplacements(ref CornerReplacementContext friendlyNewContext)
            {
                try
                {
                    // 加日志：确认补丁触发了（修复LogLevel错误）
                    Utils.MyLogger(BepInEx.Logging.LogLevel.Info, "宠物补丁触发！开始设置标签...");

                    // 处理己方宠物
                    if (skinPet.Value != -1)
                    {
                        Player friendlyPlayer = GameState.Get()?.GetPlayerBySide(Player.Side.FRIENDLY);
                        if (friendlyPlayer != null)
                        {
                            // 输出当前设置的ID（确认是不是6）
                            Utils.MyLogger(BepInEx.Logging.LogLevel.Info, $"设置己方宠物ID：{skinPet.Value}");

                            friendlyPlayer.SetTag(GAME_TAG.PET_VARIANT_ID, skinPet.Value);
                            friendlyPlayer.SetTag((GAME_TAG)184, 0); // 不隐藏

                            // 同步实体时加日志
                            SyncPetEntityTag(friendlyPlayer, skinPet.Value, "己方");
                        }
                        else
                        {
                            // 修复：LogLevel.Warn 改为 LogLevel.Warning（枚举没有Warn，正确是Warning）
                            Utils.MyLogger(BepInEx.Logging.LogLevel.Warning, "找不到己方玩家对象！");
                        }
                    }
                    ...
                    if (skinOpposingPet.Value != -1)
                    {
                        ...
                        {
                            Utils.MyLogger(BepInEx.Logging.LogLevel.Info, $"设置对手宠物ID：{skinOpposingPet.Value}");

                            opposingPlayer.SetTag(GAME_TAG.PET_VARIANT_ID, skinOpposingPet.Value);
                            opposingPlayer.SetTag((GAME_TAG)184, 0);

                            SyncPetEntityTag(opposingPlayer, skinOpposingPet.Value, "对手");
                        }
                        else
                        {
                            // 修复：Warn -> Warning
                            Utils.MyLogger(BepInEx.Logging.LogLevel.Warning, "找不到对手玩家对象！");
                        }
                    }
                }
                catch (Exception ex) // 修复：确保异常变量正确声明
                {
                    // 修复：Warn -> Warning，且ex变量存在
                    Utils.MyLogger(BepInEx.Logging.LogLevel.Error, $"宠物设置错误：{ex.Message}\n{ex.StackTrace}");
                }
            }

            // 同步宠物实体+详细日志
            private static void SyncPetEntityTag(Player player, int petId, string side)
            {
                if (player == null) return;

                // 尝试不同的PET_ENTITY_ID数值（183不对的话，试182/184/185）
                int petEntityId = player.GetTag((GAME_TAG)183); // 先试183
                Utils.MyLogger(BepInEx.Logging.LogLevel.Info, $"{side}宠物实体ID（183）：{petEntityId}");

                Entity petEntity = GameState.Get()?.GetEntity(petEntityId);
                if (petEntity != null)
                {
                    petEntity.SetTag(GAME_TAG.PET_VARIANT_ID, petId);
                    Utils.MyLogger(BepInEx.Logging.LogLevel.Info, $"{side}宠物实体设置成功！ID={petId}");
                }
                else
                {
                    // 修复：Warn -> Warning
                    int petEntityId2 = player.GetTag((GAME_TAG)182);
                    Utils.MyLogger(BepInEx.Logging.LogLevel.Warning, $"{side}宠物实体为null！尝试182获取的ID：{petEntityId2}");
                }
            }

            // 新增：在游戏真正开始后再设置一次（防止标签被覆盖）
            [HarmonyPostfix]
            [HarmonyPatch(typeof(Gameplay), "Awake")] // 游戏场景加载完成时触发
            private static void PatchGameplayAwake()
            {
                Utils.MyLogger(BepInEx.Logging.LogLevel.Info, "游戏场景加载完成，重新设置宠物标签...");
                // 重新执行一次设置逻辑
                if (skinPet.Value != -1)
                {
                    Player friendlyPlayer = GameState.Get()?.GetPlayerBySide(Player.Side.FRIENDLY);
                    friendlyPlayer?.SetTag(GAME_TAG.PET_VARIANT_ID, skinPet.Value);
                    friendlyPlayer?.SetTag((GAME_TAG)184, 0);
                }
                if (skinOpposingPet.Value != -1)
                {
                    Player opposingPlayer = GameState.Get()?.GetPlayerBySide(Player.Side.OPPOSING);
            ...
            public static void PatchLoadCard(Entity __instance, ref string cardId)
            {
                ...
                else if (Utils.CheckInfo.IsHero(cardId, out Assets.CardHero.HeroType heroType))
                {
                        ...
                        )
                    {
                        ...
                        {
                            // 1. 获取原始英雄的dbid
                            int originalDbid = GameUtils.TranslateCardIdToDbId(cardId);
                            // 2. 从映射中查询替换皮肤列表（注意类型是List<int>）
                            if (HeroesMapping.TryGetValue(originalDbid, out List<int> skinIds) && skinIds.Count > 0)
                            {
                                // 3. 检查是否为酒馆英雄（符合你的业务逻辑）
                                if (Utils.CheckInfo.IsHero(originalDbid, out Assets.CardHero.HeroType res)
                                    && res == Assets.CardHero.HeroType.BATTLEGROUNDS_HERO)
                                {
                                    // 4. 从皮肤列表中选一个（这里取第一个，也可随机：skinIds[UnityEngine.Random.Range(0, skinIds.Count)]）
                                    // int selectedSkinDbid = skinIds[0];

                                    // 关键：随机选取列表中的一个皮肤ID（替代原来的skinIds[0]）
                                    int randomIndex = UnityEngine.Random.Range(0, skinIds.Count);
                                    int selectedSkinDbid = skinIds[randomIndex];

                                    // 5. 用选中的皮肤ID生成新的cardId（核心：替换原始ID）
                                    cardId = GameUtils.TranslateDbIdToCardId(selectedSkinDbid);
                                }
                            }
                        }
                    }
                        ...
                        )
                    {
                        ...
                        if (__instance.GetCard().GetControllerSide() == Player.Side.FRIENDLY)
                        {
                            ...
                            {
                                // 计算原始英雄dbid（仅计算一次，避免冗余）
                                int originalDbid = GameUtils.TranslateCardIdToDbId(cardId);
                                // 用原始dbid查询替换皮肤列表（直接使用已计算的originalDbid）
                                if (HeroesMapping.TryGetValue(originalDbid, out List<int> skinIds) && skinIds.Count > 0)
                                {
                                    // 检查是否为有效英雄类型
                                    if (Utils.CheckInfo.IsHero(originalDbid, out Assets.CardHero.HeroType res))
                                    {
                                        // 修正逻辑：排除BATTLEGROUNDS_HERO和BATTLEGROUNDS_GUIDE（用&&）
                                        if (res != Assets.CardHero.HeroType.BATTLEGROUNDS_HERO
                                            && res != Assets.CardHero.HeroType.BATTLEGROUNDS_GUIDE)
                                        {
                                            // 从皮肤列表中选择一个替换ID（这里取第一个，也可随机：skinIds[UnityEngine.Random.Range(0, skinIds.Count)]）
                                            // int selectedSkinDbid = skinIds[0];
